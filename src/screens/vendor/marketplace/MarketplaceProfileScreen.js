...
//   const store       = useAppStore(s => s.getVendorStore(user?.id));
...
//           </View>
//           <View style={{ flex: 1 }}>
//             <Text style={s.storeName}>{store?.storeName || user?.name || 'My Store'}</Text>
//             <Text style={s.storeCat}>{store?.category || 'Marketplace Vendor'}</Text>
...
/**
 * MarketplaceProfileScreen.js — API-connected version
 *
 * Changes from original:
 *  - Store data now loaded from/saved to MySQL via Spring Boot API
 *  - Vacation mode toggle persists to DB
 *  - All existing UI, styles, navigation, and design unchanged
 *
 * Drop this file in: src/screens/vendor/marketplace/MarketplaceProfileScreen.js
 */
...
// ── API slice — persists to MySQL ─────────────────────────────────────────────
import { useVendorStoreSlice } from '../../../api/vendorStoreApi';

// ─────────────────────────────────────────────────────────────────────────────
// ── Verify Card (unchanged from original) ────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
function MarketplaceVerifyCard({ navigation }) {
  const verificationStatus = useAuthStore(s => {
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
// ── Menu Row (unchanged from original) ───────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
function MenuRow({ emoji, label, sub, onPress, rightElement, last }) {
  return (
    <View>
...
export default function MarketplaceProfileScreen({ navigation }) {
  ...
  // ── Pull from the API-backed slice (not appStore) ──────────────────────────
  ...
  const setVacationMode = useVendorStoreSlice(s => s.setVacationMode);
  const resetVendorState = useVendorStoreSlice(s => s.resetVendorState);

  // Local UI state — mirrors DB value once loaded
  const [vacation, setVacation] = useState(false);
  const [vacationSaving, setVacationSaving] = useState(false);

  // ── Load store from backend on mount (and after navigating back here) ──────
  const loadStore = useCallback(async () => {
    try {
      await fetchMyStore();
    } catch (err) {
      // 404 = no store created yet — handled silently (myStore stays null)
      if (err?.status !== 404) {
        console.warn('Could not load store:', err.message);
      }
    }
  }, [fetchMyStore]);
  ...
  // Sync vacation toggle with DB value when store loads/changes
  useEffect(() => {
    if (myStore) {
      setVacation(myStore.vacationMode ?? false);
    }
  }, [myStore]);

  // ── Vacation mode toggle — saves to DB ────────────────────────────────────
  const handleVacationToggle = async (newValue) => {
    setVacation(newValue); // optimistic UI
    setVacationSaving(true);
    try {
      await setVacationMode(newValue);
    } catch (err) {
      setVacation(!newValue); // revert on failure
      Alert.alert('Error', err.message || 'Could not update vacation mode.');
    } finally {
      setVacationSaving(false);
    }
  };

  // ── Logout — also clears vendor state from memory ─────────────────────────
  const handleLogout = () => {
    Alert.alert('Logout', 'Are you sure you want to logout?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: () => {
          resetVendorState(); // clear in-memory cache
          logout();
        },
      },
    ]);
  ...
  // ── Store summary display values ──────────────────────────────────────────
  const storeName = myStore?.storeName || user?.name || 'My Store';
  const storeCategory = myStore?.category || (myStore ? myStore.vendorType : 'Marketplace Vendor');
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Card style={s.storeCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.storeCat}>
              {hasStore ? storeCategory : 'No store yet — tap Create Store'}
            </Text>

            {/* Vacation toggle — only meaningful when store exists */}
            {hasStore && (
              <View style={s.vacationRow}>
                <Text style={s.vacationLabel}>Vacation Mode</Text>
                {vacationSaving
          ...
          {/* Edit Store / Create Store — navigates to StoreProfile */}
          <TouchableOpacity
            style={s.editBtn}
            onPress={() => navigation.navigate('StoreProfile')}
          >
            <Text style={s.editBtnText}>
              {hasStore ? 'Edit Store' : 'Create Store'}
            </Text>
          </TouchableOpacity>
        </Card>

        {/* ── Store Management section — unchanged layout ────────────────── */}
        <SectionLabel title="🏪 Store Management" />
        <Card style={s.menuCard}>
          <MenuRow emoji="🏪" label="Store Profile" sub="Name, category, address" onPress={() => navigation.navigate('StoreProfile')} />
          <MenuRow emoji="🚚" label="Delivery Settings" sub="Modes, charges, radius" onPress={() => navigation.navigate('DeliverySettings')} />
        ...
        {/* ── Account section — unchanged layout ────────────────────────── */}
        <SectionLabel title="👤 Account" />
        <Card style={s.menuCard}>
          <MenuRow emoji="❓" label="Help & Support" sub="FAQs, contact support" onPress={() => Alert.alert('Help', 'Support coming soon.')} />
          <MenuRow emoji="⭐" label="Rate the App" sub="Share your feedback" onPress={() => Alert.alert('Thanks!', 'Rating feature coming soon.')} last />
