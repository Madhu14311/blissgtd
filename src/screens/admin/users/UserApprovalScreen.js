...
function UserCard({ user, onApprove, onReject }) {
  ...
  return (
    <View style={styles.card}>
      ...
      {/* Row 3: Action buttons — for 'pending' or 'not_submitted' (Admin can bypass) */}
      {(vStatus === 'pending' || vStatus === 'not_submitted') && (
        <View style={{ marginTop: 8 }}>
          {user.documentsJson && (
            <TouchableOpacity 
              ...
              onPress={() => {
                try {
                  const docsObj = JSON.parse(user.documentsJson);
                  const docList = Object.keys(docsObj).map(k => `• ${k}: ${docsObj[k]}`).join('\n');
                  Alert.alert('Submitted Documents', docList);
                } catch(e) {
                  Alert.alert('Documents', user.documentsJson);
                }
              }}
            >
              <Text style={styles.viewDocsText}>📄 View Submitted Documents</Text>
            </TouchableOpacity>
          )}
...
const styles = StyleSheet.create({
  ...
  viewDocsBtn:  { backgroundColor: '#F1F5F9', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10, marginBottom: 10, borderWidth: 1, borderColor: '#CBD5E1', alignItems: 'center' },
  viewDocsText: { color: '#475569', fontSize: 13, fontWeight: '700' },
