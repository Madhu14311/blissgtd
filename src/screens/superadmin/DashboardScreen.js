...
export default function DashboardScreen({ navigation }) {
  const {
    societies = [],
    builders = [],
    builderRequests = [], // Added builderRequests
    adminRequests = [],
    societyRequests = [],
    projectRequests = [],
    notifications = [],
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      // You can also add other fetch calls here if needed
    }, [])
  );
  ...
  // Unified Notification Count logic
  const notificationCount =
    (builderRequests || []).filter((b) => b.status === 'Pending').length +
    (adminRequests || []).filter((a) => (a.status === 'Pending' || a.approvalStatus === 'Pending')).length +
    (societies || []).filter((s) => s.status === 'Pending').length +
    (projectRequests || []).filter((p) => (p.status === 'Pending' || p.approvalStatus === 'Pending')).length +
    (Array.isArray(superAdminPending) ? superAdminPending.length : 0) +
    (Array.isArray(notifications) ? notifications.filter(n => !n.read && (!n.targetRole || n.targetRole === 'superadmin')).length : 0);
